...
value_t SmoothedHingeLoss::compute_loss(value_t a_dot_omega, value_t label) const {
  value_t one_minus_label_times_aomega = 1 - label * a_dot_omega;
  if (one_minus_label_times_aomega <= 0.) {
    return 0;
  } else if (one_minus_label_times_aomega <= 1.) {
    return 0.5 * sq(one_minus_label_times_aomega);
  }
  return one_minus_label_times_aomega - 0.5;
}
...
value_t SmoothedHingeLoss::compute_2nd_derivative(value_t a_dot_omega,
                                             value_t label) const {
  value_t one_minus_label_times_aomega = 1 - label * a_dot_omega;
  if (one_minus_label_times_aomega <= 0.) {
...
value_t SmoothedHingeLoss::lipschitz_constant() const {
  return 1;
}
